"""
	SFD with Acetylation	(from allfluxSFD.py on Sep 4 2016) adding Acetylation functionality
	from 1 to 0 density
...
def hop(this): 	# try to hop free particle of entry this
	...
	if ran.random()<0.5:	# never leave to left ##Refers to pos>0 because particle can't go left
		...
		if pos==width-1:
			...
			x[pos]= 0
			free.pop(this)		#Removes and returns the last element of the list
			Nfree-=1		##We have one less free particle because the particle just left the system
...
def output():	#
	...
	np.save('AcetylData_{0}_{1}_{2}'.format(name,scale,nrun),scaledAcetylOccupancy)
#######################################
...
while ttime<tmax:
	...
	while thistmax >0.0:
		...
		avp[0:width] += x*dt
		acetylOccupancy[width] +=dt
		acetylOccupancy[0:width] += asite*dt
		thistmax -= dt
		ttime += dt
		nextx = ran.random()*totrate	# so nextx is where in totrate
		nextacetyl = ran.random()
		if nextx < Nfree*kon: # bind
			pos=free.pop(ran.randrange(Nfree)) #index of this
			bound.append(pos) # pop from free, add to bound ##pop removes thing at that index and returns it, so pos is now the position of the new bound particle
			Nfree  -= 1
			Nbound += 1
		...
		if nextacetyl < arate:	
			pos=(ran.randrange(Nfree+Nbound))
...
#	print("*",thisleft,thisright,nexttime,thistmax,ttime)
	...
	scaledAcetylOccupancy = acetylOccupancy/acetylOccupancy[width]
	avp[width+1] = (thisleft+thisright)/2.0
#	print("**",thisleft,thisright,nexttime,thistmax,ttime,avp[width+1])

#	if nexttime == tmax:
#		output()
#	print(ttime,nexttime,itycale,thisleft,thisright,Nbound,Nfree)

	scale += 1
	nexttime *= TFACTOR
	print(acetylOccupancy)
avpScaled = avp/avp[width]

plt.scatter(mtScaled,scaledAcetylOccupancy[0:width],marker='.',label='Acetylated')
plt.scatter(mtScaled,avpScaled[0:width],marker='o',label='Density')
plt.xlabel('Scaled Microtubule Length')
plt.ylabel('Fraction of total time')
plt.legend()
plt.title('128 sites, 16 seconds')
plt.show()
###################### END OF FILE ##############
